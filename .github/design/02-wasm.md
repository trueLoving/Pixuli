# ⚡ WASM 模块设计方案

本文档详细描述了 Pixuli 项目中 WASM（WebAssembly）模块的设计方案，包括设计目的、负责内容、架构设计、应用场景和注意事项。

---

## 🎯 设计目的

### 核心目标

- **高性能**：使用 Rust 实现图片处理功能，提供接近原生的性能
- **跨平台运行**：支持不同运行环境的部署
  - **Node 端（推荐）**：通过 NAPI-RS 编译为原生模块，在 Node.js 和 Electron 环境中运行
  - **Web 端（可选）**：编译为 WASM 格式，在浏览器中运行（需要额外配置）
  - **移动端（可选）**：编译为 WASM 格式，在 React
    Native 环境中运行（需要额外配置）
- **内存安全**：利用 Rust 的内存安全特性，避免内存泄漏和缓冲区溢出
- **类型安全**：通过 TypeScript 类型定义确保接口类型安全
- **可扩展性**：模块化设计，易于添加新功能
- **简化构建**：优先使用 NAPI 统一编译流程，减少维护成本

### 解决的问题

- **性能瓶颈**：JavaScript 处理大量图片时性能不足
- **内存管理**：需要精确控制内存使用，避免内存泄漏
- **并发处理**：支持批量图片处理，提高处理效率
- **格式支持**：支持多种图片格式的转换和处理
- **跨环境运行**：同一套代码在不同环境中运行
- **构建复杂度**：简化多平台编译流程，统一使用 NAPI

### 技术选型说明

**当前实现：基于 NAPI-RS**

项目**当前使用 NAPI-RS 实现**，编译为 Node.js 原生模块（`.node` 文件）。

- **当前状态**：
  - ✅ 已实现并运行在 Node.js/Electron 环境
  - ✅ 性能最优（原生模块）
  - ✅ 支持多平台编译（Windows、macOS、Linux）
  - ❌ 不支持纯浏览器环境
  - ❌ 不支持 React Native 移动端

- **适用场景**：
  - PC 端应用（Electron）
  - Node.js 后端服务
  - 不需要浏览器和移动端支持的情况

**推荐方案：统一使用 WASM（支持所有平台）**

如果需要同时支持 React
Native、Web 和 Electron，**推荐迁移到 WASM 统一构建方案**，只需维护一套代码和构建配置。

- **优势**：
  - ✅ **一套代码，所有平台通用**：Node.js、Electron、浏览器、React Native
  - ✅ **统一构建流程**：只需 `wasm-pack build` 命令，构建简单
  - ✅ **跨平台兼容性好**：WASM 是 Web 标准，所有现代环境都支持
  - ✅ **Node.js 原生支持**：Node.js v8.0.0+ 原生支持 WASM
  - ✅ **Electron 完美支持**：Electron 基于 Node.js，完全支持 WASM
  - ✅ **React Native 支持**：通过 WASM 运行时（如 wasmtime、wasmer）支持
  - ✅ **维护成本低**：只需维护一套代码和构建配置

- **性能说明**：
  - 性能约为 NAPI 原生模块的 80-90%（对于大多数场景足够）
  - 启动时间稍长（首次加载需要初始化 WASM 模块）
  - 可以通过预加载和缓存优化启动时间

- **限制**：
  - 某些依赖（如 ONNX Runtime）可能不支持 WASM（需要条件编译）
  - 性能略低于 NAPI（但对于图片处理场景，差异可接受）

- **适用场景**：
  - ✅ **需要支持所有平台**（React Native、Web、Electron）
  - ✅ **希望统一构建流程**
  - ✅ **性能要求不是极致**（80-90% 的性能已经足够好）

**可选方案：NAPI + WASM 混合（性能优先）**

如果需要极致性能，可以保持当前 NAPI-RS 实现，同时提供 WASM 版本：

- **优势**：
  - Electron/Node.js 使用 NAPI（性能最优，100% 性能）
  - Web/React Native 使用 WASM（兼容性最好）
  - 通过 package.json 的 exports 字段自动选择
  - 保持现有 NAPI 实现不变

- **实现方式**：
  - 保持现有 NAPI-RS 代码和构建流程
  - 添加 WASM 构建配置（使用条件编译）
  - 通过 package.json 的 exports 字段实现智能加载

- **限制**：
  - 需要维护两套构建配置
  - 构建复杂度增加
  - 代码需要条件编译（区分 NAPI 和 WASM）

- **适用场景**：
  - 对 Electron 性能要求极高
  - 需要支持 Web/React Native
  - 愿意承担额外的维护成本

**迁移建议**

如果当前只需要支持 Electron，可以继续使用 NAPI-RS。如果需要支持 Web 和 React
Native，建议：

1. **短期方案**：保持 NAPI-RS，添加 WASM 构建（混合方案）
2. **长期方案**：逐步迁移到统一 WASM 方案（简化维护）

---

## 📦 负责内容

### 核心功能模块

#### 1. 图片压缩模块 (compress)

- **WebP 压缩**：支持有损和无损 WebP 格式压缩
- **质量控制**：可调节压缩质量（0-100）
- **批量压缩**：支持多张图片批量压缩处理
- **压缩统计**：返回详细的压缩统计信息（压缩率、大小等）

#### 2. 图片格式转换模块 (convert)

- **多格式支持**：支持 JPEG、PNG、WebP、GIF、BMP、TIFF 格式转换
- **智能转换**：自动处理透明度和颜色空间
- **尺寸调整**：支持图片尺寸调整和宽高比保持
- **批量转换**：支持多张图片批量格式转换
- **转换选项**：支持质量、透明度、无损等选项配置

#### 3. 图片分析模块 (analyze)

- **基础信息获取**：
  - 获取图片尺寸、格式、通道数等信息
  - 检测图片文件格式
  - 分析图片主要颜色
- **AI 分析功能**：
  - 对象检测：检测图片中的对象和位置
  - 场景识别：识别图片场景类型
  - 标签生成：自动生成图片标签
  - 描述生成：生成图片描述文本
- **模型支持**：支持 ONNX Runtime 模型（Qwen 等，Node 端）

#### 4. 图片编辑模块 (edit)

- **裁剪功能**：
  - 矩形裁剪：支持指定区域裁剪
  - 宽高比裁剪：支持固定宽高比裁剪
  - 智能裁剪：基于内容识别的智能裁剪
- **旋转功能**：
  - 角度旋转：支持任意角度旋转
  - 90度旋转：快速旋转（90°、180°、270°）
  - 自动旋转：根据 EXIF 信息自动旋转
- **翻转功能**：
  - 水平翻转
  - 垂直翻转
- **滤镜功能**：
  - 亮度调整
  - 对比度调整
  - 饱和度调整
  - 色调调整
  - 模糊效果
  - 锐化效果
  - 灰度转换
  - 复古滤镜
  - 黑白滤镜
- **批量编辑**：支持多张图片批量应用编辑操作

### 技术特性

- **异步支持**：所有函数都支持异步调用
- **错误处理**：完善的错误处理和错误信息返回
- **类型安全**：完整的 TypeScript 类型定义
- **测试覆盖**：完整的单元测试和集成测试

---

## 🏗️ 架构设计

### 整体架构

#### 当前架构（基于 NAPI-RS）

**当前实现**使用 NAPI-RS，编译为 Node.js 原生模块：

```
┌─────────────────────────────────────────────────────────┐
│ Rust 源代码 (packages/wasm/src)                          │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ lib.rs (主入口)                                      │ │
│ │ - 使用 #[napi] 宏                                   │ │
│ │ - 当前实现                                           │ │
│ └─────────────────────────────────────────────────────┘ │
│ ↓                                                        │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ compress/ (压缩模块)                                 │ │
│ │ convert/ (转换模块)                                  │ │
│ │ analyze/ (分析模块)                                  │ │
│ │ edit/ (编辑模块)                                     │ │
│ └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
                    ↓
        ┌───────────────────────────┐
        │ NAPI-RS 编译              │
        │ - 使用 #[napi] 宏         │
        │ - 自动生成 Node.js 绑定   │
        │ - 自动生成类型定义        │
        └───────────────────────────┘
                    ↓
        ┌───────────────────────────┐
        │ 原生模块 (.node)           │
        │ - Windows: .node          │
        │ - macOS: .node            │
        │ - Linux: .node            │
        │ - 支持多架构 (x64, arm64) │
        └───────────────────────────┘
                    ↓
        ┌───────────────────────────┐
        │ 运行环境                   │
        │ - Node.js                 │
        │ - Electron (PC 端)        │
        │ - 通过 require() 加载     │
        └───────────────────────────┘
```

#### 推荐架构（统一使用 WASM - 迁移方案）

如果需要支持所有平台，推荐迁移到统一的 WASM 编译方案：

```
┌─────────────────────────────────────────────────────────┐
│ Rust 源代码 (packages/wasm/src)                          │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ lib.rs (主入口)                                      │ │
│ │ - 模块导出                                           │ │
│ │ - 公共接口                                           │ │
│ │ - 使用 #[wasm_bindgen] 宏                           │ │
│ └─────────────────────────────────────────────────────┘ │
│ ↓                                                        │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ compress/ (压缩模块)                                 │ │
│ │ - compress_to_webp()                                │ │
│ │ - batch_compress_to_webp()                         │ │
│ │ - WebPCompressOptions                               │ │
│ │ - WebPCompressResult                                │ │
│ └─────────────────────────────────────────────────────┘ │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ convert/ (转换模块)                                  │ │
│ │ - convert_image_format()                            │ │
│ │ - batch_convert_image_format()                      │ │
│ │ - FormatConversionOptions                           │ │
│ │ - FormatConversionResult                            │ │
│ │ - converters.rs (格式转换器)                         │ │
│ │ - resize.rs (尺寸调整)                               │ │
│ └─────────────────────────────────────────────────────┘ │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ analyze/ (图片分析模块)                               │ │
│ │ - get_image_info()                                  │ │
│ │ - detect_image_format()                             │ │
│ │ - analyze_dominant_colors()                         │ │
│ │ - analyze_image() (AI分析)                          │ │
│ │ - batch_analyze_images()                             │ │
│ │ - check_model_availability()                         │ │
│ │ - ImageAnalysisOptions                               │ │
│ │ - ImageAnalysisResult                                │ │
│ └─────────────────────────────────────────────────────┘ │
│ ┌─────────────────────────────────────────────────────┐ │
│ │ edit/ (图片编辑模块)                                  │ │
│ │ - crop_image()                                      │ │
│ │ - rotate_image()                                    │ │
│ │ - flip_image()                                      │ │
│ │ - apply_filter()                                    │ │
│ │ - batch_edit_images()                                │ │
│ │ - CropOptions                                       │ │
│ │ - RotateOptions                                     │ │
│ │ - FilterOptions                                     │ │
│ │ - ImageEditResult                                   │ │
│ └─────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
                    ↓
        ┌───────────────────────────┐
        │ wasm-pack 编译             │
        │ - 使用 #[wasm_bindgen] 宏 │
        │ - 自动生成 JS 绑定         │
        │ - 自动生成类型定义         │
        └───────────────────────────┘
                    ↓
        ┌───────────────────────────┐
        │ WASM 模块 (.wasm)          │
        │ - 单一 .wasm 文件          │
        │ - 跨平台通用               │
        │ - 支持所有架构             │
        └───────────────────────────┘
                    ↓
        ┌───────────┬───────────┬───────────┐
        ↓           ↓           ↓
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│ Electron     │ │ Web          │ │ React Native │
│ (PC 端)      │ │ (浏览器)      │ │ (移动端)     │
│              │ │              │ │              │
│ - Node.js    │ │ - 浏览器      │ │ - WASM 运行时│
│   环境       │ │   环境        │ │ - JSI 调用   │
│ - 直接加载   │ │ - 直接加载    │ │ - 原生性能   │
│ - 最佳性能   │ │ - Web Worker  │ │ - 跨平台     │
└──────────────┘ └──────────────┘ └──────────────┘
```

### 编译流程

#### 统一编译命令

```bash
# 安装 wasm-pack（如果未安装）
cargo install wasm-pack

# 构建 Web 版本（浏览器）
wasm-pack build --target web --out-dir pkg-web

# 构建 Node.js/Electron 版本
wasm-pack build --target nodejs --out-dir pkg-node

# 构建 React Native 版本（bundler）
wasm-pack build --target bundler --out-dir pkg-mobile

# 或者使用统一脚本（package.json）
pnpm run build:wasm        # 构建所有平台
pnpm run build:wasm:web    # 仅构建 Web
pnpm run build:wasm:node   # 仅构建 Node.js
pnpm run build:wasm:mobile # 仅构建 React Native
```

#### 编译目标说明

- **web**：浏览器环境，使用 ES 模块，适合直接在浏览器中加载
- **nodejs**：Node.js/Electron 环境，使用 CommonJS，性能优化
- **bundler**：打包工具环境（Webpack、Vite 等），适合 React
  Native 和现代构建工具

#### 智能加载配置（package.json）

通过 `exports` 字段实现智能加载，根据运行环境自动选择正确的模块：

```json
{
  "name": "pixuli-wasm",
  "version": "1.0.0",
  "type": "module",
  "exports": {
    ".": {
      "browser": "./pkg-web/pixuli_wasm.js",
      "react-native": "./pkg-mobile/pixuli_wasm.js",
      "node": "./pkg-node/pixuli_wasm.js",
      "default": "./pkg-node/pixuli_wasm.js"
    },
    "./wasm": "./pkg-web/pixuli_wasm.wasm",
    "./wasm-node": "./pkg-node/pixuli_wasm.wasm",
    "./wasm-mobile": "./pkg-mobile/pixuli_wasm.wasm"
  },
  "files": ["pkg-web/**/*", "pkg-node/**/*", "pkg-mobile/**/*"]
}
```

这样，用户只需
`import pixuliWasm from 'pixuli-wasm'`，系统会自动根据运行环境选择正确的模块。

### 技术栈

#### 核心依赖

- **Rust**：主要开发语言，提供高性能和内存安全
- **wasm-bindgen**：生成 WebAssembly 和 JavaScript 之间的绑定
- **image-rs**：Rust 图片处理核心库
- **webp**：WebP 格式支持库
- **serde**：序列化支持
- **ort**：ONNX Runtime 绑定，用于 AI 分析（需要条件编译，仅 Node.js）
- **ndarray**：多维数组处理，用于 AI 计算

#### 构建工具

- **wasm-pack**：构建和发布 WASM 包的工具
- **Cargo**：Rust 包管理器

### 项目结构

```
packages/wasm/
├── src/
│   ├── lib.rs              # 主入口，模块导出，使用 #[wasm_bindgen] 宏
│   ├── compress/
│   │   └── mod.rs          # WebP 压缩实现
│   ├── convert/
│   │   ├── mod.rs          # 格式转换主逻辑
│   │   ├── types.rs        # 类型定义
│   │   ├── converters.rs   # 格式转换器实现
│   │   └── resize.rs       # 尺寸调整逻辑
│   ├── analyze/
│   │   └── mod.rs          # 图片分析实现（合并了基础信息和AI分析）
│   └── edit/
│       ├── mod.rs          # 图片编辑主逻辑
│       ├── crop.rs         # 裁剪实现
│       ├── rotate.rs       # 旋转实现
│       ├── flip.rs         # 翻转实现
│       └── filter.rs       # 滤镜实现
├── Cargo.toml              # Rust 依赖配置
├── pkg-web/                # Web 版本构建产物（wasm-pack build --target web）
│   ├── pixuli_wasm.wasm    # WASM 二进制
│   ├── pixuli_wasm.js      # JavaScript 绑定
│   └── pixuli_wasm.d.ts    # TypeScript 类型定义
├── pkg-node/               # Node.js/Electron 版本构建产物（wasm-pack build --target nodejs）
│   ├── pixuli_wasm.wasm    # WASM 二进制
│   ├── pixuli_wasm.js      # JavaScript 绑定
│   └── pixuli_wasm.d.ts    # TypeScript 类型定义
├── pkg-mobile/             # React Native 版本构建产物（wasm-pack build --target bundler）
│   ├── pixuli_wasm.wasm    # WASM 二进制
│   ├── pixuli_wasm.js      # JavaScript 绑定
│   └── pixuli_wasm.d.ts    # TypeScript 类型定义
└── package.json            # NPM 包配置（包含智能加载配置）
```

### 从 NAPI-RS 迁移到 WASM（可选）

如果当前只需要支持 Electron，可以继续使用 NAPI-RS。如果需要支持 Web 和 React
Native，可以考虑迁移到 WASM。

#### 迁移步骤

**步骤 1：修改 Cargo.toml**

从 NAPI 依赖改为 wasm-bindgen：

```toml
# 移除或注释 NAPI 依赖
# napi = "3.0.0"
# napi-derive = "3.0.0"

# 添加 wasm-bindgen 依赖
[dependencies]
wasm-bindgen = "0.2"
```

**步骤 2：修改代码中的宏**

将 `#[napi]` 改为 `#[wasm_bindgen]`：

```rust
// 修改前（NAPI）
use napi_derive::napi;

#[napi]
pub fn compress_to_webp(image_data: Vec<u8>, options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    // 实现
}

// 修改后（WASM）
use wasm_bindgen::prelude::*;

#[wasm_bindgen]
pub fn compress_to_webp(image_data: &[u8], options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    // 实现（注意：参数类型可能需要调整）
}
```

**步骤 3：添加构建脚本**

在 package.json 中添加 WASM 构建脚本（见下方配置）

**步骤 4：测试和验证**

- 在 Node.js 环境中测试
- 在 Electron 环境中测试
- 在浏览器环境中测试
- 在 React Native 环境中测试

#### 混合方案（保持 NAPI + 添加 WASM）

如果不想完全迁移，可以同时支持 NAPI 和 WASM：

**Cargo.toml 配置**：

```toml
[dependencies]
# NAPI 依赖（条件编译）
napi = { version = "3.0.0", optional = true }
napi-derive = { version = "3.0.0", optional = true }

# WASM 依赖（条件编译）
wasm-bindgen = { version = "0.2", optional = true }

[features]
default = ["napi"]
napi = ["napi", "napi-derive"]
wasm = ["wasm-bindgen"]
```

**代码中使用条件编译**：

```rust
#[cfg(feature = "napi")]
use napi_derive::napi;

#[cfg(feature = "wasm")]
use wasm_bindgen::prelude::*;

#[cfg(feature = "napi")]
#[napi]
pub fn compress_to_webp(image_data: Vec<u8>, options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    compress_to_webp_impl(image_data, options)
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn compress_to_webp(image_data: &[u8], options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    compress_to_webp_impl(image_data.to_vec(), options)
}

// 共享实现
fn compress_to_webp_impl(image_data: Vec<u8>, options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    // 实际实现逻辑
}
```

**构建命令**：

```bash
# 构建 NAPI 版本（Electron/Node.js）
napi build --platform --release

# 构建 WASM 版本（Web/React Native）
wasm-pack build --target web --out-dir pkg-web --release
wasm-pack build --target nodejs --out-dir pkg-node --release
wasm-pack build --target bundler --out-dir pkg-mobile --release
```

### 统一构建方案详细说明（WASM）

#### 1. Cargo.toml 配置

```toml
[package]
name = "pixuli_wasm"
version = "0.1.0"
edition = "2021"

[lib]
crate-type = ["cdylib", "rlib"]

[dependencies]
wasm-bindgen = "0.2"
image = { version = "0.24", features = ["jpeg", "png", "gif", "bmp", "tiff"] }
webp = "0.2"
serde = { version = "1.0", features = ["derive"] }
serde_json = "1.0"

# 条件编译：ONNX Runtime 仅用于 Node.js（通过 feature flag）
[target.'cfg(not(target_arch = "wasm32"))'.dependencies]
ort = { version = "1.16", optional = true }

[features]
default = []
onnx = ["ort"]
```

#### 2. package.json 配置（智能加载）

```json
{
  "name": "pixuli-wasm",
  "version": "1.0.0",
  "type": "module",
  "main": "./pkg-node/pixuli_wasm.js",
  "types": "./pkg-node/pixuli_wasm.d.ts",
  "exports": {
    ".": {
      "browser": {
        "import": "./pkg-web/pixuli_wasm.js",
        "require": "./pkg-web/pixuli_wasm.js"
      },
      "react-native": "./pkg-mobile/pixuli_wasm.js",
      "node": "./pkg-node/pixuli_wasm.js",
      "default": "./pkg-node/pixuli_wasm.js"
    },
    "./wasm": "./pkg-web/pixuli_wasm.wasm",
    "./wasm-node": "./pkg-node/pixuli_wasm.wasm",
    "./wasm-mobile": "./pkg-mobile/pixuli_wasm.wasm"
  },
  "files": ["pkg-web/**/*", "pkg-node/**/*", "pkg-mobile/**/*"],
  "scripts": {
    "build": "npm run build:wasm",
    "build:wasm": "npm run build:wasm:web && npm run build:wasm:node && npm run build:wasm:mobile",
    "build:wasm:web": "wasm-pack build --target web --out-dir pkg-web --release",
    "build:wasm:node": "wasm-pack build --target nodejs --out-dir pkg-node --release",
    "build:wasm:mobile": "wasm-pack build --target bundler --out-dir pkg-mobile --release",
    "build:wasm:dev": "wasm-pack build --target web --out-dir pkg-web --dev",
    "clean": "rm -rf pkg-web pkg-node pkg-mobile"
  }
}
```

#### 3. 使用示例

**在所有平台中使用相同的代码**：

```javascript
// Electron、Web、React Native 都使用相同的导入语句
import init, { compress_to_webp, convert_image_format } from 'pixuli-wasm';

// 初始化（异步）
await init();

// 使用功能
const result = await compress_to_webp(imageData, { quality: 80 });
```

**Electron 中使用**：

```javascript
// main.js (主进程)
import init, { compress_to_webp } from 'pixuli-wasm';

await init(); // 初始化 WASM 模块

// 处理图片压缩
ipcMain.handle('compress-image', async (event, imageData, options) => {
  return await compress_to_webp(imageData, options);
});
```

**Web 中使用**：

```javascript
// browser.js
import init, { compress_to_webp } from 'pixuli-wasm';

await init(); // 初始化 WASM 模块

// 在 Web Worker 中使用（推荐）
// worker.js
import init, { compress_to_webp } from 'pixuli-wasm';

self.onmessage = async e => {
  await init();
  const result = await compress_to_webp(e.data.imageData, e.data.options);
  self.postMessage(result);
};
```

**React Native 中使用**：

```javascript
// App.js
import { useEffect, useState } from 'react';
import init, { compress_to_webp } from 'pixuli-wasm';

function App() {
  const [wasmReady, setWasmReady] = useState(false);

  useEffect(() => {
    // 初始化 WASM 模块
    init().then(() => {
      setWasmReady(true);
    });
  }, []);

  const handleCompress = async (imageUri) => {
    if (!wasmReady) return;

    // 读取图片数据
    const response = await fetch(imageUri);
    const imageData = await response.arrayBuffer();

    // 压缩图片
    const result = await compress_to_webp(
      new Uint8Array(imageData),
      { quality: 80 }
    );

    return result;
  };

  return (
    // UI 代码
  );
}
```

#### 4. 构建流程

```bash
# 1. 安装依赖
cargo install wasm-pack

# 2. 构建所有平台
pnpm run build:wasm

# 3. 或者分别构建
pnpm run build:wasm:web    # Web 版本
pnpm run build:wasm:node   # Node.js/Electron 版本
pnpm run build:wasm:mobile # React Native 版本

# 4. 发布到 npm
npm publish
```

#### 5. 优势总结

- ✅ **一套代码**：只需维护一套 Rust 代码
- ✅ **统一构建**：使用 `wasm-pack build` 即可
- ✅ **智能加载**：package.json 自动选择正确的模块
- ✅ **所有平台**：React Native、Web、Electron 全部支持
- ✅ **简单使用**：所有平台使用相同的导入语句
- ✅ **性能优秀**：WASM 性能约为原生代码的 80-90%

---

## 🚀 应用场景

### 1. 图片压缩场景

- **上传前压缩**：在上传到云端前压缩图片，减少上传时间和存储空间
- **批量压缩**：批量处理大量图片，提高处理效率
- **质量优化**：根据需求调整压缩质量，平衡文件大小和图片质量

### 2. 格式转换场景

- **格式统一**：将不同格式的图片转换为统一格式
- **兼容性处理**：转换为更兼容的格式（如 WebP 转 JPEG）
- **尺寸调整**：在转换时同时调整图片尺寸

### 3. 图片分析场景

- **信息获取**：快速获取图片基本信息（尺寸、格式等）
- **自动标签**：自动为图片生成标签，便于分类和搜索
- **内容识别**：识别图片中的对象和场景
- **智能描述**：生成图片描述，提升用户体验

### 4. 图片编辑场景

- **快速裁剪**：在应用中快速裁剪图片
- **旋转调整**：调整图片方向
- **滤镜应用**：应用各种滤镜效果，美化图片
- **批量编辑**：批量应用相同的编辑操作

### 5. 性能优化场景

- **批量处理**：使用批量 API 减少调用次数
- **并发处理**：在 Node.js 主进程或 Electron 主进程中并发处理多张图片
- **内存优化**：流式处理，避免一次性加载大量图片到内存

---

## ⚙️ 性能特点

### 压缩性能

- **压缩率**：WebP 比 JPEG 小 25-35%，比 PNG 小 25-50%
- **处理速度**：单张 2MB 图片压缩时间小于 100ms
- **内存占用**：流式处理，内存占用低
- **批量处理**：支持并发处理，提高吞吐量

### 格式转换性能

- **格式支持**：支持 6 种主流格式无缝转换
- **智能优化**：根据目标格式自动优化参数
- **并发处理**：支持并发处理多张图片

### 图片分析性能

- **基础分析**：图片信息获取速度 < 10ms
- **AI 分析**：支持 ONNX Runtime 模型
- **GPU 加速**：支持 GPU 加速（如果可用）
- **批量分析**：支持批量图片分析

### 图片编辑性能

- **编辑速度**：单张 2MB 图片编辑时间 < 200ms
- **滤镜处理**：实时滤镜预览支持
- **批量编辑**：支持批量应用编辑操作

---

## ⚠️ 注意事项

### 1. 平台支持

#### 当前实现（NAPI-RS）

- **Electron（PC 端）**：✅ 完美支持，原生模块性能最优
- **Node.js 后端**：✅ 完美支持，原生模块性能最优
- **Web（浏览器）**：❌ 不支持，需要通过 Electron 包装
- **React Native（移动端）**：❌ 不支持，需要额外 WASM 支持

#### WASM 统一方案优势（推荐迁移）

- **统一构建**：只需维护一套 Rust 代码
- **所有平台支持**：React Native、Web、Electron 全部支持
- **智能加载**：通过 package.json 的 exports 字段自动选择正确的模块
- **跨平台兼容**：WASM 是 Web 标准，所有现代环境都支持
- **构建简单**：只需 `wasm-pack build` 命令

#### 平台支持详情（WASM 方案）

- **Electron（PC 端）**：✅ 完美支持，通过 Node.js 环境加载 WASM（性能约为 NAPI 的 80-90%）
- **Web（浏览器）**：✅ 完美支持，直接加载 WASM 模块
- **React Native（移动端）**：✅ 支持，通过 WASM 运行时加载

#### Web 端和移动端使用方案

虽然 NAPI 编译的 `.node`
模块不能直接在 Web 端和移动端使用，但可以通过以下方式实现：

**方案一：通过 Electron/Node.js 桥接（推荐）**

- **Web 端**：通过 Electron 主进程调用 NAPI 模块，渲染进程通过 IPC 通信
- **移动端**：通过 Node.js 后端服务调用 NAPI 模块，移动端通过 HTTP API 调用

**方案二：编译为 WASM（需要额外配置）**

- **Web 端**：使用 `wasm-bindgen` 编译为 WASM，在浏览器中运行
- **移动端**：使用 `wasm-bindgen` 编译为 WASM，在 React
  Native 的 WASM 运行时中运行

**方案三：混合方案**

- Node.js/Electron 环境：使用 NAPI 编译的 `.node` 模块（性能更好）
- Web/移动端：使用 WASM 编译的 `.wasm` 模块（兼容性更好）

### 2. 内存管理

- **大文件处理**：对于超大文件，考虑分块处理或流式处理
- **内存限制**：注意内存使用，避免处理过多大文件导致内存溢出
- **及时释放**：处理完成后及时释放内存

### 3. 错误处理

- **输入验证**：验证输入数据格式和大小
- **错误捕获**：所有函数都应包含错误处理
- **错误信息**：提供清晰的错误信息，便于调试

### 4. 数据转换

- **类型转换**：NAPI-RS 自动处理 Rust 类型和 JavaScript 类型之间的转换
- **数据拷贝**：大文件传递时注意数据拷贝开销
- **序列化**：复杂数据结构需要序列化/反序列化

### 5. 构建配置

- **多平台构建**：NAPI-RS 自动为配置的平台构建
- **依赖管理**：注意平台特定的依赖（如 ONNX Runtime 库）
- **测试**：在不同平台上测试功能

### 6. 性能优化

- **编译优化**：使用 release 模式编译，启用 LTO
- **算法优化**：选择高效的图片处理算法
- **并发处理**：合理使用并发，避免过度并发导致资源竞争

### 7. 安全性

- **输入验证**：验证所有输入数据，防止恶意数据
- **边界检查**：检查数组边界，防止缓冲区溢出
- **资源限制**：限制处理文件大小和数量

### 8. 测试

- **单元测试**：为每个函数编写单元测试
- **集成测试**：测试与 Node.js/Electron 的集成
- **性能测试**：建立性能基准测试
- **边界测试**：测试边界情况和异常情况

---

## 📈 性能指标

### 压缩性能指标

- **压缩率**：WebP 平均压缩率 30-50%
- **处理速度**：2MB 图片压缩时间 < 100ms
- **内存占用**：单张图片处理内存占用 < 50MB
- **批量处理**：支持 100+ 图片并发处理

### 格式转换性能指标

- **转换速度**：2MB 图片转换时间 < 150ms
- **格式支持**：支持 6 种主流格式
- **质量保持**：转换后质量损失 < 5%

### 图片分析性能指标

- **基础分析速度**：图片信息获取时间 < 10ms
- **AI 分析速度**：单张图片分析时间 < 500ms（CPU）
- **准确率**：基础分析准确率 75%+
- **GPU 加速**：使用 GPU 时速度提升 5-10 倍

### 图片编辑性能指标

- **编辑速度**：2MB 图片编辑时间 < 200ms
- **滤镜处理**：实时滤镜预览延迟 < 50ms
- **批量编辑**：支持 50+ 图片批量编辑

---

## 🔒 安全性考虑

### 1. 输入验证

- **数据格式**：验证输入数据是否为有效的图片格式
- **数据大小**：限制输入数据大小，防止内存溢出
- **参数范围**：验证参数范围（如质量 0-100、角度范围等）

### 2. 内存安全

- **Rust 内存安全**：利用 Rust 的所有权系统确保内存安全
- **边界检查**：所有数组访问都进行边界检查
- **资源管理**：使用 RAII 自动管理资源

### 3. 错误处理

- **错误类型**：使用 Result 类型处理错误
- **错误传播**：NAPI-RS 自动将 Rust 错误转换为 JavaScript 错误
- **错误信息**：提供清晰的错误信息

---

## 📋 最佳实践

### 1. 代码组织

- **模块化**：按功能将代码组织到不同模块
- **可复用**：提取公共逻辑为可复用函数
- **文档化**：为公共 API 编写文档注释

### 2. 错误处理

- **统一错误类型**：使用统一的错误类型
- **错误转换**：NAPI-RS 自动将 Rust 错误转换为 JavaScript 错误
- **错误信息**：提供有意义的错误信息

### 3. 性能优化

- **算法选择**：选择高效的算法和数据结构
- **内存优化**：避免不必要的内存分配
- **并发优化**：合理使用并发，提高处理效率

### 4. 测试

- **单元测试**：为每个函数编写单元测试
- **集成测试**：测试与 Node.js/Electron 的集成
- **性能测试**：建立性能基准测试

---

## 📈 未来扩展

### 功能扩展

- **更多格式**：支持更多图片格式（AVIF、HEIC 等）
- **更多 AI 模型**：支持更多 AI 模型（YOLO、CLIP 等）
- **更多滤镜**：添加更多滤镜效果
- **视频处理**：支持视频处理和转换

### 性能优化

- **SIMD 优化**：使用 SIMD 指令加速计算
- **GPU 加速**：更多功能支持 GPU 加速
- **并行处理**：优化并行处理能力

### 开发体验

- **类型生成**：NAPI-RS 自动生成 TypeScript 类型定义
- **文档生成**：自动生成 API 文档
- **测试工具**：提供测试工具和示例

---

## 🌐 Web 端和移动端使用指南

### 重要说明

**NAPI 编译的 `.node` 模块限制**：

- `.node` 模块是 Node.js 原生模块，只能在 Node.js 环境中运行
- 不能直接在浏览器或 React Native 中加载和运行
- 需要通过桥接或编译为 WASM 的方式使用

### 方案一：通过 Electron 桥接（Web 端）

如果 Web 应用运行在 Electron 环境中，可以通过主进程桥接的方式使用 NAPI 模块：

#### 架构设计

```
┌─────────────────────────────────────────┐
│ Electron 渲染进程 (Web UI)              │
│ - 使用标准 Web API                      │
│ - 通过 IPC 与主进程通信                 │
└─────────────────────────────────────────┘
                    ↓ IPC
┌─────────────────────────────────────────┐
│ Electron 主进程                         │
│ - 加载 NAPI 模块 (.node)                │
│ - 处理图片处理请求                      │
│ - 返回处理结果                          │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│ NAPI 模块 (pixuli-wasm.node)            │
│ - 高性能图片处理                        │
│ - 原生性能                              │
└─────────────────────────────────────────┘
```

#### 实现示例

**主进程代码（main.js）**：

```javascript
const { ipcMain } = require('electron');
const pixuliWasm = require('pixuli-wasm');

// 处理图片压缩请求
ipcMain.handle('compress-image', async (event, imageData, options) => {
  try {
    const result = await pixuliWasm.compressToWebp(imageData, options);
    return { success: true, data: result };
  } catch (error) {
    return { success: false, error: error.message };
  }
});

// 处理图片转换请求
ipcMain.handle('convert-image', async (event, imageData, options) => {
  try {
    const result = await pixuliWasm.convertImageFormat(imageData, options);
    return { success: true, data: result };
  } catch (error) {
    return { success: false, error: error.message };
  }
});
```

**渲染进程代码（renderer.js）**：

```javascript
const { ipcRenderer } = require('electron');

// 调用图片压缩
async function compressImage(imageFile, options) {
  const imageData = await imageFile.arrayBuffer();
  const result = await ipcRenderer.invoke('compress-image', imageData, options);

  if (result.success) {
    return result.data;
  } else {
    throw new Error(result.error);
  }
}

// 调用图片转换
async function convertImage(imageFile, options) {
  const imageData = await imageFile.arrayBuffer();
  const result = await ipcRenderer.invoke('convert-image', imageData, options);

  if (result.success) {
    return result.data;
  } else {
    throw new Error(result.error);
  }
}
```

#### 优势

- **原生性能**：使用 NAPI 模块，性能最优
- **统一代码**：只需维护一套 Rust 代码
- **简单集成**：通过 Electron IPC 即可使用

#### 限制

- **仅限 Electron**：只能在 Electron 应用中使用
- **IPC 开销**：需要进程间通信，有一定性能开销

### 方案二：通过 Node.js 后端服务（移动端）

如果移动端应用需要图片处理功能，可以通过 Node.js 后端服务提供 API：

#### 架构设计

```
┌─────────────────────────────────────────┐
│ React Native 移动端                     │
│ - 发送 HTTP 请求                        │
│ - 接收处理结果                          │
└─────────────────────────────────────────┘
                    ↓ HTTP/WebSocket
┌─────────────────────────────────────────┐
│ Node.js 后端服务                        │
│ - Express/Koa 服务器                    │
│ - 加载 NAPI 模块                        │
│ - 提供 REST API                         │
└─────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────┐
│ NAPI 模块 (pixuli-wasm.node)            │
│ - 高性能图片处理                        │
│ - 原生性能                              │
└─────────────────────────────────────────┘
```

#### 实现示例

**后端服务代码（server.js）**：

```javascript
const express = require('express');
const multer = require('multer');
const pixuliWasm = require('pixuli-wasm');

const app = express();
const upload = multer({ storage: multer.memoryStorage() });

// 图片压缩 API
app.post('/api/compress', upload.single('image'), async (req, res) => {
  try {
    const imageData = req.file.buffer;
    const options = JSON.parse(req.body.options || '{}');

    const result = await pixuliWasm.compressToWebp(imageData, options);
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});

// 图片转换 API
app.post('/api/convert', upload.single('image'), async (req, res) => {
  try {
    const imageData = req.file.buffer;
    const options = JSON.parse(req.body.options || '{}');

    const result = await pixuliWasm.convertImageFormat(imageData, options);
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});

app.listen(3000, () => {
  console.log('Server running on port 3000');
});
```

**移动端代码（React Native）**：

```javascript
import { uploadImage } from './api';

// 调用图片压缩
async function compressImage(imageUri, options) {
  const formData = new FormData();
  formData.append('image', {
    uri: imageUri,
    type: 'image/jpeg',
    name: 'image.jpg',
  });
  formData.append('options', JSON.stringify(options));

  const response = await fetch('http://your-server:3000/api/compress', {
    method: 'POST',
    body: formData,
  });

  const result = await response.json();
  if (result.success) {
    return result.data;
  } else {
    throw new Error(result.error);
  }
}
```

#### 优势

- **原生性能**：后端使用 NAPI 模块，性能最优
- **统一代码**：只需维护一套 Rust 代码
- **灵活部署**：后端可以部署在服务器或本地

#### 限制

- **网络开销**：需要网络传输，有延迟
- **需要后端**：需要维护 Node.js 后端服务

### 方案三：编译为 WASM（所有平台）

如果需要统一支持所有平台（Node.js、浏览器、React
Native），可以将 Rust 代码编译为 WASM。**WASM 模块可以在 Node.js 环境中运行**（Node.js
v8.0.0+ 原生支持）。

#### 架构设计

```
┌─────────────────────────────────────────┐
│ Rust 源代码                             │
│ - 使用条件编译区分 NAPI 和 WASM         │
└─────────────────────────────────────────┘
        ↓                    ↓
┌──────────────┐    ┌──────────────────┐
│ NAPI 编译    │    │ WASM 编译         │
│ (Node.js)    │    │ (Web/移动端)      │
└──────────────┘    └──────────────────┘
```

#### 实现步骤

**1. 修改 Cargo.toml，添加 WASM 支持**：

```toml
[lib]
crate-type = ["cdylib", "rlib"]

[dependencies]
# NAPI 依赖（条件编译）
napi = { version = "3.0.0", optional = true }
napi-derive = { version = "3.0.0", optional = true }

# WASM 依赖（条件编译）
wasm-bindgen = { version = "0.2", optional = true }

[features]
default = ["napi"]
napi = ["napi", "napi-derive"]
wasm = ["wasm-bindgen"]
```

**2. 修改代码，支持条件编译**：

```rust
// lib.rs
#[cfg(feature = "napi")]
use napi_derive::napi;

#[cfg(feature = "wasm")]
use wasm_bindgen::prelude::*;

#[cfg(feature = "napi")]
#[napi]
pub fn compress_to_webp(image_data: Vec<u8>, options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    // NAPI 实现
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn compress_to_webp(image_data: &[u8], options: Option<WebPCompressOptions>) -> Result<WebPCompressResult> {
    // WASM 实现（相同逻辑）
}
```

**3. 构建 WASM 版本**：

```bash
# 安装 wasm-pack
cargo install wasm-pack

# 构建 WASM 版本
wasm-pack build --target web --out-dir pkg-web

# 构建 React Native 版本
wasm-pack build --target bundler --out-dir pkg-mobile
```

**4. 在 Node.js 中使用**：

```javascript
// Node.js 环境（v8.0.0+）
const { readFileSync } = require('fs');
const { WASI } = require('wasi');
const {
  initSync,
  compress_to_webp,
} = require('pixuli-wasm/pkg-node/pixuli_wasm.js');

// 初始化 WASM 模块（同步方式）
initSync();

// 或者使用异步方式
// const init = require('pixuli-wasm/pkg-node/pixuli_wasm.js');
// await init();

async function compressImage(imagePath, options) {
  const imageData = readFileSync(imagePath);
  const result = compress_to_webp(imageData, options);
  return result;
}
```

**5. 在 Web 端使用**：

```javascript
import init, { compress_to_webp } from 'pixuli-wasm/pkg-web/pixuli_wasm.js';

async function compressImage(imageData, options) {
  // 初始化 WASM 模块
  await init();

  // 调用 WASM 函数
  const result = compress_to_webp(imageData, options);
  return result;
}
```

**6. 在 React Native 中使用**：

```javascript
import { loadWasmModule } from 'react-native-wasm';
import { compress_to_webp } from 'pixuli-wasm/pkg-mobile/pixuli_wasm';

async function compressImage(imageData, options) {
  // 加载 WASM 模块
  await loadWasmModule('pixuli-wasm/pkg-mobile/pixuli_wasm.wasm');

  // 调用 WASM 函数
  const result = compress_to_webp(imageData, options);
  return result;
}
```

#### 优势

- **统一构建**：一套代码，所有平台通用
- **直接运行**：可以在 Node.js、浏览器和 React Native 中直接运行
- **无需桥接**：不需要 Electron 或后端服务（浏览器和移动端）
- **离线支持**：可以在离线环境中使用
- **跨平台兼容**：Node.js v8.0.0+ 原生支持 WASM

#### 限制

- **性能较低**：WASM 性能不如 NAPI 原生模块（约 10-20% 性能差异）
- **初始化开销**：首次加载需要初始化 WASM 模块
- **依赖限制**：某些依赖（如 ONNX Runtime）可能不支持 WASM
- **内存限制**：WASM 内存管理相对复杂

#### NAPI vs WASM 性能对比（Node.js 环境）

| 指标       | NAPI 原生模块    | WASM 模块 | 说明              |
| ---------- | ---------------- | --------- | ----------------- |
| 启动速度   | 快               | 较慢      | WASM 需要初始化   |
| 运行性能   | 100%             | 80-90%    | WASM 有性能开销   |
| 内存占用   | 低               | 稍高      | WASM 需要额外内存 |
| 构建复杂度 | 中等             | 简单      | WASM 构建更统一   |
| 跨平台支持 | Node.js/Electron | 所有平台  | WASM 支持更广     |

### 方案选择建议

| 场景                        | 推荐方案              | 原因                       |
| --------------------------- | --------------------- | -------------------------- |
| Electron 应用（PC 端）      | 方案一：Electron 桥接 | 性能最优，集成简单         |
| React Native 应用（移动端） | 方案二：Node.js 后端  | 性能最优，统一代码         |
| 纯 Web 应用（浏览器）       | 方案三：WASM          | 可以直接运行               |
| 需要离线支持                | 方案三：WASM          | 无需网络连接               |
| 需要最佳性能                | 方案一/二：NAPI       | 原生性能                   |
| 需要统一构建（所有平台）    | 方案三：WASM          | 一套代码，所有平台         |
| Node.js 后端服务            | NAPI 或 WASM          | NAPI 性能更好，WASM 更简单 |

---

## 📝 总结

WASM 模块是 Pixuli 项目的核心性能组件，通过 Rust 实现提供了高性能的图片处理能力。

### 当前实现

项目**当前使用 NAPI-RS 实现**，编译为 Node.js 原生模块（`.node`
文件），完美支持 Electron 和 Node.js 环境，性能最优。

**当前实现特点**：

- 使用 `#[napi]` 宏标记函数
- 通过 `napi build` 命令编译
- 生成 `.node` 原生模块文件
- 支持 Windows、macOS、Linux 多平台
- 性能最优（原生模块）

### 未来方向

如果需要同时支持 React Native、Web 和 Electron，有以下选择：

1. **混合方案**：保持 NAPI-RS 用于 Electron/Node.js，添加 WASM 用于 Web/React
   Native
   - 优势：Electron 保持最佳性能，同时支持 Web/React Native
   - 成本：需要维护两套构建配置

2. **统一方案**：迁移到 WASM 统一构建，一套代码支持所有平台
   - 优势：统一构建流程，维护成本低
   - 成本：性能略低于 NAPI（80-90%），但对于图片处理场景足够

**推荐**：

- **如果只需要 Electron**：继续使用 NAPI-RS，性能最优
- **如果需要多平台支持**：考虑迁移到 WASM 或使用混合方案

### WASM 统一方案的优势（如果迁移）

1. **统一代码库**：只需维护一套 Rust 代码，所有平台共享
2. **统一构建流程**：使用 `wasm-pack build` 即可为所有平台构建
3. **智能加载**：通过 package.json 的 exports 字段，自动根据运行环境选择正确的模块
4. **跨平台兼容**：WASM 是 Web 标准，所有现代环境都原生支持
5. **性能优秀**：虽然略低于 NAPI（80-90%），但对于图片处理场景已经足够

设计充分考虑了跨平台兼容性、内存安全和性能优化。当前通过 NAPI-RS 实现了与 Node.js/Electron 环境的无缝集成，如果未来需要支持更多平台，可以考虑迁移到 WASM 统一方案。

---

## 📚 相关文档

- [跨端资源共享设计方案](./01-cross-platform-resources.md) - 了解跨端资源共享

---

最后更新：2025年11月
