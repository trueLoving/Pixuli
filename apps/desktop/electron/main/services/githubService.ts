...
export function registerGithubHandlers() {
  ...
  ipcMain.handle('github:setAuth', async (event, token: string) => {
    try {
      githubService.octokit = new Octokit({ auth: token });
      return { success: true };
    } catch (error) {
  ...
  ipcMain.handle('github:upload', async (event, params: any) => {
    try {
      ...
      try {
        const existingFile = await githubService.octokit.rest.repos.getContent({
          owner,
          repo,
          path: filePath,
          ref: branch,
  ...
  ipcMain.handle('github:delete', async (event, params: any) => {
    try {
      ...
      // 获取文件的当前 SHA
      const fileInfo = await githubService.octokit.rest.repos.getContent({
        owner,
        repo,
        path: filePath,
        ref: branch,
      ...
      await githubService.octokit.rest.repos.deleteFile({
        owner,
        repo,
        path: filePath,
  ...
  ipcMain.handle('github:getList', async (event, params: any) => {
    try {
      ...
      const imageFiles = response.data.filter(item =>
        githubService.isImageFile(item.name)
      );
      ...
      try {
        ...
        const graphqlResponse = (await githubService.octokit.graphql(query, {
          owner,
          repo,
        ...
        try {
          const allCommits = await githubService.octokit.rest.repos.listCommits(
            {
              owner,
              repo,
              per_page: 1,
          ...
          if (allCommits.data.length > 0) {
            // 获取完整的提交历史来找到第一次提交
            const fullHistory =
              await githubService.octokit.rest.repos.listCommits({
                owner,
                repo,
      ...
      try {
        ...
        const metadataResponse =
          await githubService.octokit!.rest.repos.getContent({
            owner,
            repo,
            path: `${path}/.metadata`,
            ref: branch,
  ...
  ipcMain.handle('github:updateMetadata', async (event, params: any) => {
    try {
      ...
      try {
        const fileResponse = await githubService.octokit!.rest.repos.getContent(
          {
            owner,
            repo,
            path: metadataPath,
