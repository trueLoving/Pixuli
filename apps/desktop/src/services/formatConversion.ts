...
export class FormatConversionService {
  ...
  static async batchConvertImages(
    ...
    try {
      ...
      // 调用WASM批量转换函数
      const results = await window.wasmAPI.batchConvertImageFormat(imagesData, options)
      ...
      return results.map((result, index) => {
        ...
        const convertedFile = this.createConvertedFile(result.data, file.name, options.targetFormat)
   ...
   * 验证转换选项
   */
  private static validateConversionOptions(originalFormat: string, options: FormatConversionOptions): void {
