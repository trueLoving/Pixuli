import { ImageItem, GiteeConfig, ImageUploadData } from '@packages/ui/src';
...
export class GiteeStorageService {
  ...
  private async setAuth() {
    try {
      await (window as any).giteeAPI.giteeSetAuth(this.config.token);
    } catch (error) {
  ...
  async uploadImage(uploadData: ImageUploadData): Promise<ImageItem> {
    try {
      ...
      // ========== 上传图片文件（同时上传元数据） ==========
      // 主进程会在上传图片后自动上传元数据，与 GitHub 逻辑保持一致
      const response = await (window as any).giteeAPI.giteeUpload({
  ...
  async deleteImage(imageId: string, fileName: string): Promise<void> {
    try {
      await (window as any).giteeAPI.giteeDelete({
        owner: this.config.owner,
        repo: this.config.repo,
        path: this.config.path,
        branch: this.config.branch,
  ...
  async getImageList(): Promise<ImageItem[]> {
    try {
      const response = await (window as any).giteeAPI.giteeGetList({
        owner: this.config.owner,
        repo: this.config.repo,
        path: this.config.path,
        branch: this.config.branch,
  ...
  async updateImageInfo(
    ...
    try {
      ...
      await (window as any).giteeAPI.giteeUpdateMetadata({
        owner: this.config.owner,
        repo: this.config.repo,
        path: this.config.path,
        branch: this.config.branch,
