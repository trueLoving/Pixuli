...
import { X, Keyboard, Command, Search, RefreshCw, Zap, ArrowRightLeft, Brain, Settings } from 'lucide-react'
import { keyboardManager, SHORTCUT_CATEGORIES } from '@/utils/keyboardShortcuts'
...
const KeyboardHelpModal: React.FC<KeyboardHelpModalProps> = ({ isOpen, onClose }) => {
  ...
  const getCategoryIcon = (categoryName: string) => {
    switch (categoryName) {
      case SHORTCUT_CATEGORIES.GENERAL:
        return <Command className="w-5 h-5" />
      case SHORTCUT_CATEGORIES.SEARCH:
        return <Search className="w-5 h-5" />
      case SHORTCUT_CATEGORIES.HELP:
        return <Keyboard className="w-5 h-5" />
      case SHORTCUT_CATEGORIES.MODAL:
        return <X className="w-5 h-5" />
      case SHORTCUT_CATEGORIES.IMAGE_BROWSER:
        return <RefreshCw className="w-5 h-5" />
      case SHORTCUT_CATEGORIES.IMAGE_EDIT:
