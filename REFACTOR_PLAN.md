...
| 主应用收敛 | `apps/pixuli` 为三端唯一主应用；RN 已迁入 `archive/apps/mobile/`（§1.9.2 ✅）                        |
...
├─────────────────────────────────────────────────────────────────┤
│  archive/apps/mobile（Expo RN，只读）  ← REF-513 已归档 │
...
- [x] REF-509 真机冒烟通过（启动、配置、列表、上传）— **P6 工程签收**见
      [#166](https://github.com/trueLoving/Pixuli/issues/166) ✅ ·
      [14-capacitor-android-smoke-acceptance.md](docs/02-system-design/14-capacitor-android-smoke-acceptance.md)
- [x] REF-510 相机/相册/文件等 L3 在 Capacitor 有等价实现或**书面降级**说明（[#120](https://github.com/trueLoving/Pixuli/issues/120)
      ·
      [13-capacitor-native-plugins.md](docs/02-system-design/13-capacitor-native-plugins.md)）
- [x] REF-511 拍照元数据在 Capacitor 路线可采集（[#141](https://github.com/trueLoving/Pixuli/issues/141)
      ·
      [13-capacitor-native-plugins.md §6](docs/02-system-design/13-capacitor-native-plugins.md)）
- [x] REF-508「仅 RN」能力清单已决策：实现 / 放弃 / 延期（[#119](https://github.com/trueLoving/Pixuli/issues/119)
      ·
...
**归档步骤**（2026-06-17 已完成 1–3、5；步骤 4 移除
`release-mobile.yml`，Capacitor APK 发版见 #153）：

1. [x] `apps/mobile` → `archive/apps/mobile/`（保留只读历史，不删 git 记录）
2. [x] `pnpm-workspace.yaml` 不再包含 `apps/mobile`；根 `package.json` 删除
       `dev:mobile` / `build:mobile:rn`
3. [x] README / AGENTS / 签收文档：Mobile 实现改为 Capacitor，RN 标 **archived**
4. [x] CI 移除 Expo/RN `tsc` 与 `release-mobile.yml`；Capacitor `v*-android`
       发版 workflow 见 **#153**
5. [x] `archive/README.md` 补充归档日期与替代路径
...
**建议顺序**：**#164** ✅ → **#150** ✅ → **#165** ✅ → **#119** ✅ → **#120**
✅ → **#141** ✅ → **#161** ✅ → **#166** ✅ → **#152/#153** →
**#151**（RN 主体已归档，Wiki 见 #153）。REF-607 下一步：**P7**（Gitee 代理退役）；**#159**
✅（`imageStore` local 模式）；**#160** ✅（Web OPFS/FSA）；**#161**
...
**REF-508** — 盘点 `apps/mobile` 与 `@pixuli/ui/web` 重叠；交付
[12-ui-native-migration-assessment.md](docs/02-system-design/12-ui-native-migration-assessment.md)（#119
✅）：**现阶段仅 Capacitor 功能对齐**；RN 与 `./native` **不维护**，#151
**整体归档**。
...
**#165** ✅ → **#119** ✅ → **#120** ✅ → **#141** ✅ → **#166** ✅ →
**#152/#153** → **#151**（收尾 Wiki）。REF-607 **#156→#158** ✅、**#159**
✅、**#160** ✅、**#161** ✅；下一步
**P7**（代理退役与 local-only 收官、Capacitor APK CI）。
...
| 移动端拍照元数据              | REF-511 ✅ → [#141](https://github.com/trueLoving/Pixuli/issues/141) · [13-capacitor-native-plugins.md §6](docs/02-system-design/13-capacitor-native-plugins.md)                   |
