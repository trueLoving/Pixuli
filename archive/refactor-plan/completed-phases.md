...
> **收官同步**：2026-06-29 — REF-516 / REF-607 父 Issue（#163 /
> #144）P0～P7 全 ✅
...
| P7   | 工程整理 + CI APK + Wiki              | [#152](https://github.com/trueLoving/Pixuli/issues/152)、[#153](https://github.com/trueLoving/Pixuli/issues/153)、[#151](https://github.com/trueLoving/Pixuli/issues/151) | ✅   |

**收官标准（已满足）**：`apps/pixuli` 为 Web + Desktop + Capacitor
Mobile 唯一交付路径；`archive/apps/mobile` 只读归档；`release-android.yml` 产出
`v*-android` APK；用户手册 §4.3 含安装说明。
...
**收官标准（已满足）**：三端 `WorkspaceAdapter` + `workspaceStore`
local 模式；Gitee `/proxy/*` 与 `remote-only` 已移除；预览仅 local，公网分享走
`buildPublicUrl` 直链。
