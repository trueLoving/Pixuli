...
export function PWAInstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (dismissed) {
      ...
      if (Date.now() - parseInt(dismissed, 10) < oneDay) setShowPrompt(false);
    }
    const updateDismissed = localStorage.getItem('pwa-update-dismissed');
    if (updateDismissed) {
      const oneDay = 24 * 60 * 60 * 1000;
      if (Date.now() - parseInt(updateDismissed, 10) < oneDay)
        setDismissedUpdate(true);
    }
  }, []);
