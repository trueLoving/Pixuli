...
describe('useVirtualScroll', () => {
  ...
  describe('可见项目生成', () => {
    ...
    it('应该在滚动时更新可见项目', () => {
      ...
      // 新的可见项目应该包含更大的索引
      expect(Math.min(...newVisibleItems)).toBeGreaterThan(Math.min(...initialVisibleItems))
    })
  })
  ...
  describe('性能优化', () => {
    ...
    it('当items变化时应该重新计算', () => {
      ...
      const { result, rerender } = renderHook(
        ({ items }) => useVirtualScroll(items, { itemHeight: 50, containerHeight: 500 }),
  ...
  describe('容器高度处理', () => {
    it('应该使用提供的初始容器高度', () => {
      ...
      expect(result.current.endIndex - result.current.startIndex + 1).toBeLessThanOrEqual(visibleCount + 1)
    })
