# Pixuli 产品需求规格说明书（基线）

- **文档版本**：2.4
- **创建日期**：2025-01-27
- **最后更新**：2026-09-12（v2.4：与资源库 UI SSOT
  v3.7 对齐——同步→复制链接；废止用户向「发布」）
- **适用分支**：`main`（M1～M3 已交付为阶段一；§2.1a 为现行产品需求）
- **项目名称**：Pixuli — 资源工作区客户端
- **读者**：产品、开发、测试、运维

---

## 一、文档概述

### 1.1 目的

本文档为 **M1～M3 重构后基线** 的单一产品需求规格说明，涵盖：

- 产品底线、范围与 M1 裁剪
- 功能与非功能需求及验收状态
- 架构摘要与路线图索引

### 1.2 范围

- 产品经理、项目经理：需求理解与排期
- 开发工程师：功能实现与接口设计
- 测试工程师：测试用例设计与验收标准
- 运维与部署：运行环境与约束

### 1.3 术语与缩写

| 术语              | 说明                                                                                               |
| ----------------- | -------------------------------------------------------------------------------------------------- |
| PRS               | 产品需求规格说明书（本文档）                                                                       |
| PWA               | 渐进式 Web 应用（Progressive Web App）                                                             |
| CRUD              | 创建 / 读取 / 更新 / 删除                                                                          |
| Monorepo          | 单一代码库管理多个包/应用                                                                          |
| Provider / 连接器 | 存储插件实现（`StorageProvider`），对接一类云或 Git 服务                                           |
| Registry          | `StoragePluginRegistry`，注册与创建 Provider                                                       |
| 资源 / Asset      | 工作区内一条文件（图片、PDF、视频等）+ 元数据                                                      |
| 访问策略          | 外人能否打开链接：由资源位置（本机/远端）+ 仓库/云策略决定；主路径为**复制链接**（非库内「发布」） |

### 1.4 功能需求表列说明

各 **§5** 需求表统一列：

| 列           | 含义                                                                            |
| ------------ | ------------------------------------------------------------------------------- |
| **ID**       | 需求唯一标识                                                                    |
| **需求**     | 产品应交付的能力描述（验收依据）                                                |
| **优先级**   | P0 / P1 / P2；已移除项标「—」                                                   |
| **平台**     | 全平台 = Web + Desktop + Mobile；或单列 Web / Desktop / Mobile                  |
| **实现情况** | 相对当前 `main` 基线的实现状态：✅ 已实现 · ⏳ 待实现/待完善 · 部分 · ❌ 已移除 |

---

## 二、产品底线与范围

### 2.1 产品底线（不变）

| 项              | 说明                                                                                     |
| --------------- | ---------------------------------------------------------------------------------------- |
| **交付形态**    | **Web（含 PWA）**、**Desktop（Electron）**、**Mobile（Capacitor Android）** 三端继续维护 |
| **插件边界**    | 云与 Git 存储经 `StorageProvider` 插件扩展；**core/provider 禁止依赖 UI（`src/ui`）**    |
| **官方 Server** | **不提供**官方 NestJS / 自建后端为一等公民；`server/` 已归档，仅社区或自建对接           |
| **对外主张**    | 本地优先的资源工作区；AI 分析 / 自动标签 / 批处理仍为增强能力，不替代资源库主路径        |

### 2.1a 需求升级（2026-08 · 现行产品需求）

**阶段一（已交付 / 仍为 `main` 实现基线）**：图片管理 + 本地工作区 + **GitHub /
Gitee** 同步与公网直链。

**阶段二（现行需求，验收以本小节 + §5.8 为准，实现 ⏳）**：

| 项               | 需求                                                                                                                                                                                                     |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **资源类型**     | 不限于图片；至少覆盖 **图片、PDF、视频** 等常见文件，在同一工作区浏览与管理                                                                                                                              |
| **存储连接**     | 通过连接器对接不同云/Git 服务，官方规划：**GitHub、Gitee、OneDrive、Google Cloud**（Drive / GCS 等，以插件 manifest 为准）；更多服务走同一 Provider 契约                                                 |
| **本地 + 远程**  | **本地工作区为 SSOT**（浏览、搜索、预览默认读本地）；远程为可选同步目标（push / pull）；外链靠**复制链接**，不设用户向「发布」流程                                                                       |
| **对外访问控制** | 用户可按资源设置对外访问（私有 / 可外链等）。**远程侧若该云支持** ACL、分享链接或公开 URL，则在 UI 内配置；**若不支持**，对应选项禁用，并给出**明确提示**（原因 + 可替代做法），禁止静默失败或假装已授权 |
| **UI**           | 三端共用；信息架构见 [04-asset-library-ui.md](./04-asset-library-ui.md)                                                                                                                                  |

阶段一能力不回退：GitHub/Gitee
**同步 + 复制公网链接**路径在阶段二落地前仍须可用。

### 2.2 M1 展示裁剪（已从产品移除）

以下能力在 **M1（REF-101～107）** 已从主应用移除，**不再作为需求验收项**。明细见
[04-backlog.md](../04-backlog.md)（REF-402）。

| 已移除能力                       | 说明                                                 |
| -------------------------------- | ---------------------------------------------------- |
| 幻灯片（Slideshow）              | 独立路由与播放器                                     |
| 时间线（Timeline）               | 按时间分组浏览                                       |
| 照片墙（Photo Wall）             | 增强展示组件                                         |
| 3D 画廊（Gallery 3D）            | 增强展示组件                                         |
| 浏览模式多路由切换               | 文件/幻灯片/时间线 Tab；现统一为 **资源库网格/列表** |
| 占位页 analyze / edit / generate | 未接入主流程的独立路由（重定向至资源库）             |

**当前主导航（产品信息架构）**：

- **阶段一（已实现）**：**资源库浏览**（网格/列表与预览）；**图片工具**（`/compress`、`/convert`）；**设置 /
  GitHub·Gitee 连接**。
- **阶段二（目标 IA）**：**资源库 / 连接 / 同步与复制链接 / 工具 / 设置**，见
  [04-asset-library-ui.md](./04-asset-library-ui.md)（废止用户向「发布」）。

### 2.3 不在官方范围内的能力

| 项                                  | 处理方式                                                                                 |
| ----------------------------------- | ---------------------------------------------------------------------------------------- |
| `packages/wasm`、WASM IPC           | 已从仓库移除；Web/桌面图片处理以 Canvas 等为主                                           |
| `benchmark/`                        | 已归档，非主构建路径                                                                     |
| `server/`（NestJS）                 | 已移出 workspace；**非官方交付物**                                                       |
| `packages/common`（pixuli-common）  | M3 REF-311 已删除；由 `@pixuli/core` + `app/src/ui`（`@/ui`）+ `@pixuli/provider-*` 替代 |
| 官方 Upyun 等未列入阶段二清单的图床 | 非官方 P0；可经同一 `StorageProvider` 扩展，见 backlog                                   |

### 2.4 当前架构与包（产品视角）

```text
app     Web + Desktop + Android（Capacitor）共用（Vite + React + Electron + android/）
packages/core   类型、工具、StoragePluginRegistry 契约
app/src/ui  三端共享 DOM UI（`@/ui`）
packages/plugin-provider-github | plugin-provider-gitee  官方存储插件
```

### 2.5 规划中能力（里程碑）

| 里程碑     | 产品向交付                                   | 计划 Issue                                                                                                                                    |
| ---------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **M4**     | PRD/README/Wiki/CHANGELOG 与现架构一致       | REF-401～415                                                                                                                                  |
| **M5**     | PWA 深化、Desktop L3；Capacitor 三端共享 PoC | REF-501～510                                                                                                                                  |
| **M6**     | 三端交互规范、UI、性能边界、标签/AI、批处理  | REF-601～605；[03](./03-three-platform-interaction-spec.md) · [04](./04-asset-library-ui.md)                                                  |
| **阶段二** | 多类型资源、多云连接器、本地+远程访问控制    | 需求 §2.1a / §5.8；UI [04-asset-library-ui.md](./04-asset-library-ui.md)；插件 [03-plugin-system.md](../02-system-design/03-plugin-system.md) |

详见 [PLANS.md](../../PLANS.md)。

---

## 三、项目概述

### 3.1 背景

Pixuli 源于两类需求：

1. **实际需求**：博客与网站运营中统一的图片存储、压缩与批量处理需求。
2. **技术实践**：在已有 Vue3 图片管理应用经验基础上，使用 React 技术栈重新实现，并扩展至多平台（Web、桌面、移动端）。

### 3.2 产品定位

Pixuli 是一款**本地优先的资源工作区客户端**（三端共用 `app`）：

- **Web**：Vite + React，支持 PWA
- **Desktop**：Electron，与 Web 同一套 UI
- **Mobile**：Capacitor Android（Expo RN 已归档）

**核心价值**：

- 管理 **多种资源**（图片、PDF、视频等），本地工作区为 SSOT
- 用 **连接器** 对接 GitHub、Gitee、OneDrive、Google Cloud 等；官方不提供 NestJS
  Server
- **控制对外访问**：本地始终可用；需要外链时先同步再**复制链接**（能否打开取决于云/仓库策略）
- 阶段一已交付路径：图片 + GitHub/Gitee 同步与公网直链（仍保留）

### 3.3 目标用户

- 个人博客 / 静态站：稳定公网直链（Git 公开仓 + 复制链接）
- 内容与知识工作者：图片、PDF、视频同库管理，按需同步到个人云
- 需要把部分资源分享到外网、其余保持私有的创作者与开发者

---

## 四、系统架构与技术栈

### 4.1 项目结构

```
Pixuli/
├── app/                     # Web + Desktop + Android（含 src/ui）
├── packages/
│   ├── core/                # @pixuli/core — 类型、工具、插件 Registry 契约
│   ├── plugin-provider-github/
│   └── plugin-provider-gitee/
├── docs/                    # 产品 / 系统设计 / 业务设计文档
└── PLANS.md         # 重构与 Issue 追踪
```

### 4.2 技术栈概览

| 层级       | 技术                       | 说明                                             |
| ---------- | -------------------------- | ------------------------------------------------ |
| 前端框架   | React 19 + TypeScript      | Web/Desktop 统一 UI                              |
| 构建工具   | Vite                       | Web/Desktop 构建；Desktop 叠加 Electron          |
| 桌面运行时 | Electron                   | 跨平台桌面                                       |
| 移动端     | Capacitor Android          | 与 Web/Desktop 共用 `app` DOM UI；Expo RN 已退役 |
| 状态管理   | Zustand                    | imageStore / workspaceStore 等                   |
| 存储插件   | StorageProvider + Registry | 官方 GitHub/Gitee provider 包                    |
| 图片处理   | Canvas API                 | Web/Desktop/Capacitor 渲染进程                   |
| 云存储     | GitHub API / Gitee API     | 预览走本地工作区；复制链接用 `publicUrl`         |

### 4.3 平台能力矩阵

> 幻灯片/时间线等已从产品移除（§2.2）。下表为**重构后目标态**；⏳ 项见 M5/M6。

| 能力                           | Web  | Desktop                    | Mobile        |
| ------------------------------ | ---- | -------------------------- | ------------- |
| 多仓库源（GitHub/Gitee 插件）  | ✅   | ✅                         | ✅            |
| 资源库网格/列表 + 预览         | ✅   | ✅                         | ✅            |
| 图片 CRUD（单张）              | ✅   | ✅                         | ✅            |
| 批量上传 / 批量删除            | ✅   | ✅                         | ✅            |
| 批量元数据 / 批处理增强        | ⏳   | ⏳                         | ⏳ M6 REF-605 |
| 标签与描述 + AI 分析           | 部分 | 部分（Desktop 有 AI 基础） | ⏳ M6 REF-604 |
| 搜索与筛选                     | ✅   | ✅                         | ✅            |
| 操作日志                       | ✅   | ✅                         | ✅            |
| PWA（安装/离线壳）             | ✅   | —                          | —             |
| 图片处理工具（压缩/转换等）    | 部分 | 部分                       | 部分          |
| Desktop 离线队列/自动更新/托盘 | —    | ⏳ M5                      | —             |
| 大规模列表性能边界             | ⏳   | ⏳                         | ⏳ M6 REF-603 |
| 相机 / 相册                    | —    | —                          | ✅            |

---

## 五、功能需求

### 5.1 仓库源管理

**概述**：用户可配置多个 GitHub 或 Gitee 仓库作为图片存储源，并可在源之间切换。M3 起通过
`StoragePluginRegistry` 与 `StoredSourceEntry`（含 `pluginId`）管理。

| ID           | 需求                                                           | 优先级 | 平台   | 实现情况 |
| ------------ | -------------------------------------------------------------- | ------ | ------ | -------- |
| F-SOURCE-001 | 支持添加多个 GitHub 仓库源（owner、repo、branch、token、path） | P0     | 全平台 | ✅       |
| F-SOURCE-002 | 支持添加多个 Gitee 仓库源（结构同上）                          | P0     | 全平台 | ✅       |
| F-SOURCE-003 | 支持在已配置源之间切换为当前活跃源                             | P0     | 全平台 | ✅       |
| F-SOURCE-004 | 支持编辑与删除已有仓库源                                       | P0     | 全平台 | ✅       |
| F-SOURCE-005 | 配置持久化（Web/桌面：localStorage；移动：AsyncStorage）       | P0     | 全平台 | ✅       |
| F-SOURCE-006 | 配置表单校验（必填项、格式等）                                 | P1     | 全平台 | ✅       |
| F-SOURCE-007 | 配置导入、导出与清空                                           | P1     | 全平台 | ✅       |

**业务规则**：GitHub 与 Gitee 配置可并存；Token 等敏感信息仅本地持久化。

---

### 5.2 图片 CRUD

#### 5.2.1 创建

| ID         | 需求                                         | 优先级 | 平台   | 实现情况 |
| ---------- | -------------------------------------------- | ------ | ------ | -------- |
| F-CRUD-C01 | 单张图片上传并带元数据（名称、描述、标签等） | P0     | 全平台 | ✅       |
| F-CRUD-C02 | 批量图片上传并显示进度（进度条/百分比）      | P0     | 全平台 | ✅       |
| F-CRUD-C03 | 相机拍摄与相册选择上传                       | P0     | Mobile | ✅       |

#### 5.2.2 读取

| ID         | 需求                                                 | 优先级 | 平台   | 实现情况 |
| ---------- | ---------------------------------------------------- | ------ | ------ | -------- |
| F-CRUD-R01 | 从当前活跃源加载图片列表                             | P0     | 全平台 | ✅       |
| F-CRUD-R02 | 以网格/列表形式展示图片                              | P0     | 全平台 | ✅       |
| F-CRUD-R03 | 全屏预览，支持缩放与左右切换                         | P0     | 全平台 | ✅       |
| F-CRUD-R04 | 查看单张图片元数据（名称、描述、标签、尺寸、时间等） | P1     | 全平台 | ✅       |
| F-CRUD-R05 | 下拉刷新以同步最新列表                               | P1     | 全平台 | ✅       |
| F-CRUD-R06 | 元数据本地缓存，减少重复请求、优化列表加载与离线浏览 | P1     | 全平台 | ✅       |

#### 5.2.3 更新 / 删除

| ID         | 需求                               | 优先级 | 平台   | 实现情况 |
| ---------- | ---------------------------------- | ------ | ------ | -------- |
| F-CRUD-U01 | 编辑图片元数据，支持重命名冲突检测 | P0     | 全平台 | ✅       |
| F-CRUD-U02 | 对多张图片批量编辑元数据           | P1     | 全平台 | ⏳       |
| F-CRUD-D01 | 单张图片删除并带确认机制           | P0     | 全平台 | ✅       |
| F-CRUD-D02 | 批量选择与批量删除                 | P0     | 全平台 | ✅       |

---

### 5.3 资源库浏览（L2：网格 / 列表）

**概述**：资源库主界面仅负责**浏览与预览**当前工作区资源；**不含**压缩/转换等图片处理工具（见§5.6），**不含**连接表单逻辑（见 §5.1）。历史路由名可能仍含
`/photos`。

| ID          | 需求                                                                                 | 优先级 | 平台   | 实现情况  |
| ----------- | ------------------------------------------------------------------------------------ | ------ | ------ | --------- |
| F-BROWSE-01 | 网格/列表展示当前源图片，点击预览，支持左右切换                                      | P0     | 全平台 | ✅        |
| F-BROWSE-02 | 幻灯片模式（自动/手动播放、过渡动画等）                                              | —      | —      | ❌ 已移除 |
| F-BROWSE-03 | 时间线模式（按时间分组浏览）                                                         | —      | —      | ❌ 已移除 |
| F-BROWSE-04 | 全平台提供仓库源设置入口，支持 GitHub / Gitee 仓库的配置、切换、多源管理与设置页访问 | P0     | 全平台 | ✅        |

> 原 F-BROWSE-04（侧栏含工具入口）、F-BROWSE-05（移动端资源库）已合并为本条：工具导航归入§5.6；列表能力归入 F-BROWSE-01。幻灯片/时间线已移除，见
> [backlog](../04-backlog.md)。

---

### 5.4 操作日志

| ID       | 需求                                               | 优先级 | 平台   | 实现情况 |
| -------- | -------------------------------------------------- | ------ | ------ | -------- |
| F-LOG-01 | 记录上传、删除、编辑、配置变更、批量上传等关键操作 | P1     | 全平台 | ✅       |
| F-LOG-02 | 提供日志查看界面，支持按类型、时间、关键词等筛选   | P1     | 全平台 | ✅       |
| F-LOG-03 | 展示统计（如按类型操作数、今日数量、成功率）       | P2     | 全平台 | ✅       |
| F-LOG-04 | 支持将日志导出为 JSON/CSV 文件                     | P2     | 全平台 | ✅       |
| F-LOG-05 | 三端操作日志能力一致（移动端可为弹窗等差异化呈现） | P2     | 全平台 | ✅       |

---

### 5.5 PWA 与离线（Web）

| ID       | 需求                          | 优先级 | 平台 | 实现情况 |
| -------- | ----------------------------- | ------ | ---- | -------- |
| F-PWA-01 | Service Worker 注册与离线缓存 | P1     | Web  | ✅       |
| F-PWA-02 | 安装提示（PWAInstallPrompt）  | P1     | Web  | ✅       |
| F-PWA-03 | 离线状态指示                  | P1     | Web  | ✅       |
| F-PWA-04 | PWA 更新检测与提示            | P2     | Web  | ⏳       |

---

### 5.6 图片处理工具

**概述**：压缩、格式转换、编辑、分析、生成等**独立于资源库浏览**的工具能力；各端通过独立路由或工具页访问（如
`/compress`、`/convert`）。需求范围为**全平台**；各端实现技术可不同（Canvas
/ 原生等）。

#### 5.6.1 工具入口与导航

| ID            | 需求                                                         | 优先级 | 平台   | 实现情况 |
| ------------- | ------------------------------------------------------------ | ------ | ------ | -------- |
| F-TOOL-NAV-01 | 全平台提供图片处理工具入口（压缩、转换等），与资源库浏览分离 | P1     | 全平台 | 部分     |

#### 5.6.2 格式转换

| ID            | 需求                              | 优先级 | 平台   | 实现情况 |
| ------------- | --------------------------------- | ------ | ------ | -------- |
| F-TOOL-CVT-01 | 支持 JPEG、PNG、WebP 等格式间转换 | P0     | 全平台 | 部分     |

#### 5.6.3 图片压缩

| ID            | 需求                                          | 优先级 | 平台   | 实现情况 |
| ------------- | --------------------------------------------- | ------ | ------ | -------- |
| F-TOOL-CMP-01 | 可调压缩质量（如 10%–100%），支持预览压缩效果 | P0     | 全平台 | 部分     |

#### 5.6.4 图片编辑

| ID            | 需求                                               | 优先级 | 平台   | 实现情况 |
| ------------- | -------------------------------------------------- | ------ | ------ | -------- |
| F-TOOL-EDT-01 | 裁剪、缩放、旋转及元数据编辑等与处理相关的编辑能力 | P0     | 全平台 | 部分     |

#### 5.6.5 图片分析

| ID            | 需求                                       | 优先级 | 平台   | 实现情况 |
| ------------- | ------------------------------------------ | ------ | ------ | -------- |
| F-TOOL-ANZ-01 | 基础信息提取与扩展分析（标签、描述生成等） | P1     | 全平台 | ⏳       |

#### 5.6.6 图片生成

| ID            | 需求                                           | 优先级 | 平台   | 实现情况 |
| ------------- | ---------------------------------------------- | ------ | ------ | -------- |
| F-TOOL-GEN-01 | 基于文本或条件生成图片（含 Dify 等工作流集成） | P2     | 全平台 | ⏳       |

> **实现情况说明**：Mobile 端压缩/转换等能力较完整；Web/Desktop 工具页已就绪，部分处理逻辑待与 UI 完全打通，故标「部分」。

---

### 5.7 搜索与筛选

| ID          | 需求                               | 优先级 | 平台   | 实现情况 |
| ----------- | ---------------------------------- | ------ | ------ | -------- |
| F-SEARCH-01 | 按名称、描述、标签搜索图片         | P0     | 全平台 | ✅       |
| F-SEARCH-02 | 按标签、尺寸、日期等维度筛选       | P1     | 全平台 | ✅       |
| F-SEARCH-03 | 多种排序方式（如时间、名称、大小） | P1     | 全平台 | ✅       |
| F-SEARCH-04 | 多条件组合、搜索历史与高级搜索     | P2     | 全平台 | ⏳       |

---

### 5.8 国际化与主题

| ID         | 需求                                          | 优先级 | 平台   | 实现情况 |
| ---------- | --------------------------------------------- | ------ | ------ | -------- |
| F-I18N-01  | 中英文界面切换，文案随语言变化                | P0     | 全平台 | ✅       |
| F-THEME-01 | 亮色/暗色主题切换（含跟随系统，视端能力而定） | P1     | 全平台 | ✅       |

---

### 5.8a 资源库、多云连接与访问控制（阶段二 · 现行需求）

UI 与验收故事见
[04-asset-library-ui.md](./04-asset-library-ui.md)。阶段一需求（§5.1～§5.4）不删除。

#### 资源类型

| ID         | 需求                                                      | 优先级 | 平台   | 实现情况 |
| ---------- | --------------------------------------------------------- | ------ | ------ | -------- |
| F-ASSET-01 | 工作区可管理多种资源，至少包括图片、PDF、视频             | P0     | 全平台 | ⏳       |
| F-ASSET-02 | 按类型预览：图片网格/大图；PDF 预览或打开；视频播放或占位 | P0     | 全平台 | 部分     |
| F-ASSET-03 | 类型不适用的工具（如对 PDF 做图片压缩）须禁用并提示       | P1     | 全平台 | ⏳       |

#### 存储连接（本地 + 远程）

| ID        | 需求                                                          | 优先级 | 平台   | 实现情况 |
| --------- | ------------------------------------------------------------- | ------ | ------ | -------- |
| F-CONN-01 | 本地工作区为浏览 SSOT；远程连接可选                           | P0     | 全平台 | 部分     |
| F-CONN-02 | 连接器对接 GitHub、Gitee（已有）；规划 OneDrive、Google Cloud | P0     | 全平台 | 部分     |
| F-CONN-03 | 添加连接时展示该云能力（同步 / 公开 URL / 分享链接 / ACL 等） | P0     | 全平台 | ⏳       |
| F-CONN-04 | 可选择同步方向（远程→本地 / 本地→远程）；多连接时指定目标     | P0     | 全平台 | 部分     |

#### 对外访问控制

| ID          | 需求                                                                                             | 优先级 | 平台   | 实现情况                                      |
| ----------- | ------------------------------------------------------------------------------------------------ | ------ | ------ | --------------------------------------------- |
| F-ACCESS-01 | 可按资源配置对外访问：至少「仅本地（私有）」与「远程可访问」                                     | P0     | 全平台 | ⏳                                            |
| F-ACCESS-02 | **远程**：若当前云支持公开 URL、分享链接或对象 ACL，则在 UI 内设置                               | P0     | 全平台 | ⏳                                            |
| F-ACCESS-03 | **远程不支持**时：对应选项禁用，并提示原因与替代（如改用其他连接）                               | P0     | 全平台 | ⏳                                            |
| F-ACCESS-04 | 已同步且有公网 URL 时可复制链接；仅本地/无连接有引导；私有仓可复制并附注（无「撤销发布」假能力） | P1     | 全平台 | ✅ 部分（见 04-asset-library-ui §8.4 / P0-9） |

---

### 5.9 快捷键与帮助（Web / Desktop）

| ID       | 需求                                                            | 优先级 | 平台          | 实现情况 |
| -------- | --------------------------------------------------------------- | ------ | ------------- | -------- |
| F-KBD-01 | 常用操作快捷键（Esc 关闭弹窗、F5 刷新、配置、搜索、操作日志等） | P1     | Web / Desktop | ✅       |
| F-KBD-02 | 快捷键帮助弹窗（如 F1 打开 KeyboardHelpModal）                  | P2     | Web / Desktop | ✅       |

---

### 5.10 Web/桌面扩展（规划）

代表需求：F-WEB-DESK-01～17（布局优化、离线队列、自动更新、托盘、AI、性能边界等）。多数为 ⏳；Desktop
L3 见 M5，体验见 M6。完整 ID 列表见历史 PRD 修订记录或
[04-backlog.md](../04-backlog.md)。

---

### 5.11 移动端扩展（规划）

代表需求：F-MOBILE-02～16（批量元数据、布局、离线、手势、AI 等）。F-MOBILE-01（批量上传）已并入§5.2
F-CRUD-C02；其余多为 ⏳。

---

### 5.12 官方服务端（已移出产品范围）

Pixuli **官方不提供** NestJS Server。`server/` 已归档。客户端默认路径为
**本地工作区 + StorageProvider 连接器**（阶段一：GitHub /
Gitee；阶段二：OneDrive、Google Cloud 等）。社区/自建见
[04-backlog.md](../04-backlog.md)。

---

## 六、非功能需求

### 6.1 性能

| ID         | 需求                                         |
| ---------- | -------------------------------------------- |
| NF-PERF-01 | 多图场景列表可接受（懒加载、分页或虚拟滚动） |
| NF-PERF-02 | Web/桌面以 Canvas 为主；WASM 已归档          |
| NF-PERF-03 | 元数据本地缓存（全平台，对应 F-CRUD-R06）    |
| NF-PERF-04 | 超大规模库须定义性能边界（M6 REF-603）       |

### 6.2 安全与隐私

| ID        | 需求                   |
| --------- | ---------------------- |
| NF-SEC-01 | Token 仅存用户设备本地 |
| NF-SEC-02 | 自建后端非官方交付     |

### 6.3 兼容性

| ID           | 需求                                                                          |
| ------------ | ----------------------------------------------------------------------------- |
| NF-COMPAT-01 | Web：现代浏览器（Chrome、Firefox、Safari、Edge 等）；图片处理以 Canvas 等为主 |
| NF-COMPAT-02 | 桌面：Windows 10/11 x64；macOS 10.15+（x64/ARM64）                            |
| NF-COMPAT-03 | 移动端：Android 5.0+（API 21）；iOS 待定                                      |

### 6.4 可用性与可维护性

| ID          | 需求                                                                       |
| ----------- | -------------------------------------------------------------------------- |
| NF-USAB-01  | 主流程支持中英双语，关键操作有反馈                                         |
| NF-MAINT-01 | 共享逻辑在 `@pixuli/core`；UI 在 `app/src/ui`；存储在 `@pixuli/provider-*` |

---

## 七、存储与扩展

### 7.1 连接器（StorageProvider）

- **阶段一（已交付）**：`@pixuli/provider-github`、`@pixuli/provider-gitee`（仓库同步 +
  `buildPublicUrl` / 复制链接）
- **阶段二（规划）**：OneDrive、Google
  Cloud 等官方或社区 provider，同一 Registry 契约
- 配置因云而异（Token / OAuth / bucket）；多连接由
  `sourceStore`（或后继连接模型）管理
- **能力位**须能驱动 UI：不支持的访问控制不得静默当作已开启（F-ACCESS-03）
- Gitee 公网分享走 `buildPublicUrl` 直链（REF-607 P7；Host 代理已退役）

### 7.2 非官方扩展

| 扩展方式               | 说明                                           |
| ---------------------- | ---------------------------------------------- |
| 第三方 StorageProvider | 实现 Registry 契约后注册（REF-308）            |
| 自建 HTTP API / Server | 非官方；记入 [04-backlog.md](../04-backlog.md) |
| AI 分析                | Desktop 本地模型；产品化见 M6 REF-604          |

---

## 八、需求优先级

- **P0**：核心流程；缺失则产品不可用或体验严重受损。
- **P1**：重要功能，影响体验与差异化。
- **P2**：增强功能，可后续迭代。

---

## 九、附录

### 9.1 相关文档

| 文档                                                                             | 说明                                              |
| -------------------------------------------------------------------------------- | ------------------------------------------------- |
| [02-product-user-manual.md](02-product-user-manual.md)                           | 产品使用手册（Wiki 源稿）                         |
| [03-three-platform-interaction-spec.md](./03-three-platform-interaction-spec.md) | 三端壳层与手势（REF-601）                         |
| [04-asset-library-ui.md](./04-asset-library-ui.md)                               | 资源库 UI SSOT：连接 / 同步 / 复制链接 / 本地操作 |
| [05-brand-visual-ui.md](./05-brand-visual-ui.md)                                 | 品牌色与状态 UI                                   |
| [04-backlog.md](../04-backlog.md)                                                | 已移除/延后需求                                   |
| [05-release-versioning.md](../02-system-design/05-release-versioning.md)         | 版本与发版制度（REF-409）                         |
| [PLANS.md](../../PLANS.md)                                                       | 重构计划与 Issue                                  |
| [03-plugin-system.md](../02-system-design/03-plugin-system.md)                   | 存储插件技术设计（Pixuli 插件体系）               |
| [项目 README](https://github.com/trueLoving/Pixuli/blob/main/README.md)          | 仓库概览                                          |

### 9.2 修订历史

| 版本 | 日期       | 变更                                                                                                            |
| ---- | ---------- | --------------------------------------------------------------------------------------------------------------- |
| 1.0  | 2025-01-27 | 初稿 PRD                                                                                                        |
| 2.0  | 2026-05-27 | REF-401：三端底线、M1 裁剪、无官方 Server、core/ui/provider                                                     |
| 2.1  | 2026-06-06 | 合并原 `03-Product-Scope-And-Cut-List.md` 为 §二；统一为基线产品需求规格说明书                                  |
| 2.2  | 2026-06-06 | 需求表「需求/实现情况」列；F-BROWSE-04 合并；工具归 §5.6；全平台范围校正（R06、BROWSE-01、工具/筛选/i18n/主题） |
| 2.3  | 2026-08-13 | 需求升级 §2.1a / §5.8a：多类型资源、多云连接、本地+远程访问控制（云不支持则 UI 提示）                           |

---

_本文档随项目迭代更新；与 [PLANS.md](../../PLANS.md) 冲突时以重构计划为准。_
