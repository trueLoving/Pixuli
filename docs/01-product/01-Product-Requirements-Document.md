...
浏览模式对**同一套当前源图片列表**提供三种展示形态，数据来自
[图片 CRUD](../03-business-design/02-image-crud.md)，不拉新数据、不写回存储。完整业务目标、规则与流程图见
[03-browse-mode 浏览模式 - 业务设计](../03-business-design/03-browse-mode.md)。

| 流程           | 说明                                                                                                                              |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **模式切换**   | 用户通过侧边栏选择「文件 / 幻灯片 / 时间线」→ 更新当前模式状态 → 路由/视图跳转至对应页面 → 用当前列表渲染该模式。                 |
| **文件模式**   | 进入文件模式 → 使用当前图片列表做网格/列表展示 → 用户点击某张 → 打开全屏预览 → 支持左右切换、缩放 → 关闭回到列表。                |
| **幻灯片模式** | 进入幻灯片 → 使用当前列表全屏展示 → 自动轮播（可配置间隔）或手动切换 → 支持循环、过渡动画与播放控件（播放/暂停、上一张/下一张）。 |
| **时间线模式** | 进入时间线 → 按时间字段（如 createdAt/updatedAt）分组 → 按组展示，组内为图片列表，便于按时间维度浏览。                            |

三端差异：Web/Desktop 支持三种模式；移动端以幻灯片为主（F-BROWSE-05），文件/时间线以产品与实现为准。各模式仅消费图片 CRUD 提供的列表，与
[仓库源管理](../03-business-design/01-repository-source-management.md)
的当前源一致。
