...
| 当前源下的图片列表加载、分页或懒加载策略                           | 图片处理（压缩、格式转换、裁剪）的算法与契约（见 [01-Three-Platform-Capability-Sharing §第二部分](../02-system-design/01-Three-Platform-Capability-Sharing.md#第二部分-跨端图片处理)） |
| 元数据的结构、校验、持久化（随图片存仓库或 Server）                | AI 分析/生成、搜索与筛选的详细规则（可另文档约定）                                                                                                                                     |
| 上传进度、错误提示、操作日志记录                                   | 图床网格/列表 UI（M1 已移除幻灯片/时间线多模式，见 [backlog](../backlog.md)）                                                                                                          |
...
| 存储插件与 Registry                             | `@pixuli/core`（`StoragePluginRegistry`）+ `@pixuli/provider-*`  |
| 各端 imageStore / sourceStore                   | `apps/pixuli`、`apps/mobile` 内 store（调用 Provider）           |
| 图片项与元数据类型                              | `@pixuli/core` 类型定义                                          |
...
- [01-Three-Platform-Capability-Sharing §第二部分 跨端图片处理](../02-system-design/01-Three-Platform-Capability-Sharing.md#第二部分-跨端图片处理)（处理契约，与 CRUD 存储配合）
