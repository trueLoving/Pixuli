...
| J1-05 | 配置导入 JSON            | ✅        | ✅  | ✅             | **已有** |                                                                                           |
| J1-06 | 配置导出 JSON            | ✅        | —   | ✅             | **实现** | RN 无导出 UI；`buildPluginConfigExport` 已 import 未接 — **#165**                         |
...
| J3-07 | 独立 filter Tab          | —         | △ 死路由         | —              | **放弃** | `filter.tsx` 已 `href: null`；Capacitor 用 Photos 内嵌搜索 — **#165** 删死代码 |
...
| 优先级 | 任务                     | 旅程 ID      | 验收要点                                                 |
| ------ | ------------------------ | ------------ | -------------------------------------------------------- |
| **P0** | 批量删除可用性确认       | J6-02        | 窄屏多选 → 删除 2 张 → 列表更新（pixuli 已有，回归即可） |
| **P0** | 批量上传入口             | J4-02        | `multiple` + `uploadMultipleImages` + 进度条             |
| **P0** | 配置导出 JSON            | J1-06        | GitHub/Gitee Modal 导出与 Web 一致                       |
| **P1** | 删除 `filter.tsx` 死路由 | J3-07        | 无隐藏重复页；仅 Photos 搜索入口                         |
| **P1** | Gitee 生产代理验证       | J1-02, L3-05 | Android release 包图片可加载                             |
| **P2** | 上传裁剪/压缩默认策略    | J4-05, J4-06 | 与产品确认是否默认开启                                   |
| **P2** | 主题设置（可选）         | J7-02        | 对齐 RN `ThemeModal` 或书面降级                          |
| **P2** | pull-to-refresh（可选）  | J2-04        | 与 REF-601「步骤一致」权衡                               |
...
| Issue            | 范围                   | 来自本文                   | 决策摘要                                                  |
| ---------------- | ---------------------- | -------------------------- | --------------------------------------------------------- |
| **#119** REF-508 | RN 组件不必迁入清单    | §四 全表                   | 确认 EmptyState 等 `@pixuli/ui/native` 在 Capacitor 可删  |
| **#120** REF-510 | Capacitor 原生插件选型 | J4-03, J4-04, J5-05, L3-05 | Camera、File、Share、Permissions；`isNativeMobile()` 分支 |
| **#141** REF-511 | 拍照元数据             | J4-08                      | EXIF/GPS/localPath 写入上传 DTO                           |
...
| #120 相机/相册 L3      | §四、§六 #120                                                    | ⏳ 待实现                    |
| #141 拍照元数据        | J4-08                                                            | ⏳ 待实现或书面延期          |
| #119 仅 RN 能力决策    | §四                                                              | ✅ **本文已决策**            |
