...
| **P6 Mobile**      | SAF / Capacitor 文件插件                                           | Mobile 适配器（`Directory.Data` 沙箱，`mobile://`；`@capacitor/clipboard` 复制链接；SAF → #120） | REF-510          | [#161](https://github.com/trueLoving/Pixuli/issues/161) ✅ |
...
**P7**（三端工作区就绪后执行，见 [§8.1](#81-gitee-图片代理退役p7)）— **父 Issue
[#144](https://github.com/trueLoving/Pixuli/issues/144) 已关闭（2026-06-29）**。
...
| **Mobile（Capacitor）** | SAF / 应用沙箱          | P3         | P6 PoC：`Directory.Data` 沙箱 + `mobile://`；SAF → #120 |
...
> 浏览模式已在 P5（#174）移除**，现仅 `local` 工作区列表；旧 `remote-only`
> localStorage 键在 `workspaceStore.initialize` 时清理。

1. 检测 `sourceStore.sources.length > 0` 且
   `workspaceStore.needsWorkspaceSetup()`（`mode === 'unset'`）。
2. 引导：**选择本地目录**（Desktop）或 **OPFS / 连接文件夹**（Web）→
   **导入绑定**：每条 `StoredSourceEntry` → `config.bindings[]`（`pickWorkspace`
   / `initialize` 时执行）。
3. 提供 **「从远端全量 pull 一次」**（可选勾选 `pullAfter`，耗时提示）。
4. 完成后
   `workspaceMode: 'local'`；应用内照片页仅展示本地 vault 列表，远端仅用于 push/pull 同步。

**应用内入口**：照片页 `needsSetup && sources.length > 0`
→ 迁移向导；`needsSetup && sources.length === 0` → 首次设置面板。
...
| Desktop PoC：选目录 + 本地列表 + push/pull 一次   | §九 P2～P3        | ✅ P2～P3              |
| URL：本地 vs 远端公网                             | §八、§8.1         | ✅ P7 收官             |
| Gitee 代理退役                                    | §8.1              | ✅ **P7**（#173）      |
| 迁移说明                                          | §十三             | ✅ P4                  |
...
- 分阶段：[#155 P0](https://github.com/trueLoving/Pixuli/issues/155) ·
  ...
  [#173 P7](https://github.com/trueLoving/Pixuli/issues/173)
