...
| 原路径                                   | 现路径 | 说明               |
| ---------------------------------------- | ------ | ------------------ |
| Gitee `proxy/constants.js`（REF-607 P7） | 已删除 | 代理子路径整段移除 |
...
3. **导入约定**：包内相对导入默认**无扩展名**；经 `package.json` exports 被 Vite
   SSR / Node 直载的模块，子依赖宜用**包路径**，避免 `./foo`
   在 monorepo 根外解析失败。
4. **工具链例外**：新增 `.js`/`.mjs`/`.cjs` 须在 PR 中说明并更新本文 §2.2。

---

## 四、与插件 Host 集成的关系

Gitee 图片 Host 代理已于 REF-607 P7（#173）退役。
