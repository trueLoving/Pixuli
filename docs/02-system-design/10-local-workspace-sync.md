...
- `syncPush`：批量映射为现有 `uploadImage` / `deleteImage` /
  `updateImageMetadata`。
- `buildPublicUrl`：复用现有 raw
  URL 逻辑；Gitee 继续走代理基址（`VITE_GITEE_PROXY_ORIGIN` /
  Desktop 本地代理）。
- `getRawUrl`：**保留**，语义为「当前配置下远端 raw」；本地预览由 `LocalVault`
  生成 `blob:` / `file:` URL。
...
| 类型           | 来源                                   | 用途                                   |
| -------------- | -------------------------------------- | -------------------------------------- |
| `local`        | `WorkspaceAdapter` + `blob:` / `file:` | 本机预览，**不**作为对外 Markdown 链接 |
| `remote-raw`   | `buildPublicUrl`                       | GitHub raw、Gitee 直连（若可用）       |
| `remote-proxy` | Gitee + 站点 `/api/gitee-proxy`        | Web/Capacitor 生产环境                 |

**UI（REF-602 / REF-601）**：「复制链接」子菜单三项；未绑定远端时仅显示
`local`（或禁用公网项）。
...
| **P4 应用切换**    | `imageStore` 在 local 模式走 `LocalVault`；保留 remote-only       | 特性开关 / 迁移向导                                    | P3、REF-507 可选 | [#159](https://github.com/trueLoving/Pixuli/issues/159)    |
...
| 模块             | 改动                                                                |
| ---------------- | ------------------------------------------------------------------- |
| `imageStore`     | `loadImages` 分支：`local` → `vault.list()`；`remote-only` → 现逻辑 |
...
4. 完成后 `workspaceMode: 'local'`；保留「切换回仅远端浏览」隐藏开关至 P4 结束。
