 ...
 */
export function formatFileSizeChinese(bytes: number, decimals: number = 2): string {
 ...
 * @returns 是否在范围内
 */
export function isFileSizeInRange(fileSize: number, minSize?: number, maxSize?: number): boolean {
 ...
 * @returns 范围描述字符串
 */
export function getFileSizeRangeDescription(minSize: number, maxSize: number): string {
 ...
 * @returns 转换后的数值
 */
export function convertFileSize(bytes: number, unit: string, decimals: number = 2): number {
  const units = {
    'B': 1,
    'KB': 1024,
    'MB': 1024 * 1024,
 ...
 * @returns 百分比值（0-100）
 */
export function getFileSizeProgress(currentSize: number, maxSize: number): number {
