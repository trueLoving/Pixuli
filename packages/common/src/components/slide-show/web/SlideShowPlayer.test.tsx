...
vi.mock('../../../locales', () => ({
  defaultTranslate: (key: string, locales?: Record<string, any>) => {
    const translations: Record<string, string> = {
      ...
      'slideShow.player.noImages': '没有图片',
      ...
      'slideShow.player.controls.export': '导出',
      'slideShow.export.title': '导出幻灯片',
      'slideShow.export.format': '格式',
      'slideShow.export.formatHTML': 'HTML',
      'slideShow.export.formatPPT': 'PPT',
...
describe('SlideShowPlayer', () => {
  ...
  describe('导出功能', () => {
    ...
    it('应该可以选择导出格式', () => {
      ...
      const pptOption = screen.getByLabelText('PPT');

      expect(htmlOption).toBeInTheDocument();
      expect(pptOption).toBeInTheDocument();
    });
  });
